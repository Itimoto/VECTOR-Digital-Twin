function [decBits, eqSym, chanEst] = helperIndoorRayTracingRxProcessing(rxSig, CIR, chanInfo, ...
...
for i = 1:numOFDMSymbols
    % Reshape for SIMO (2nd dimension is cut)
    dims = size(chanEst(:,i,:,:));
    currChanEst = reshape(chanEst(:,i,:,:), [dims(1), dims(3:end)]);

    [eqSym(:,i,:)] = ofdmEqualize(rxOFDM(:,i,:), ...
            currChanEst,'Algorithm','ZF');
end
