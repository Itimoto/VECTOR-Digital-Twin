%% Basic DoA + Positioning
% With Lunar Surface Imported
...
%% 3D Lunar Scenario
% Specify environment filename (STL format) (pulled from https://jthatch.com/Moon2STL/)
...
rx.AntennaPosition = [285.5; 0; 5.4]; % Place RX Antenna at the origin, or wherever we need, really.  
tx.AntennaPosition = [R_a*cos(deg2rad(beta_a - 90)); R_a*sin(deg2rad(beta_a - 90)); 5.4];% Place TX Antenna some distance away
tx.AntennaPosition = tx.AntennaPosition + rx.AntennaPosition - [0; 0; rx.AntennaPosition(3)]; % Place TX relative to RX, but don't mess with the Height
...
%tri = mapping.loadGeoTIFF(filename, rx.AntennaPosition);
%siteviewer(SceneModel=tri,ShowEdges=false,ShowOrigin=false);
siteviewer(SceneModel=filename, ShowEdges=false);
show(tx,"ShowAntennaHeight",false)
show(rx,"ShowAntennaHeight",false)
pattern(rx, fc, "Size",0.2); % For a ULA, the array is facing the direction of the main lobe, flaring out at the sides
...
pm = propagationModel("raytracing", ...
    ...
    "MaxNumReflections",10, ...          % No reflections - LOS only
