...
fc = 5.5e9; % Carrier frequency
lambda = physconst("lightspeed")/fc;

% ULA: [4, 1]. URA: [4 4]
txArray = arrayConfig("Size",[1 1],"ElementSpacing",2*lambda);
rxArray = arrayConfig("Size",[1 4],"ElementSpacing",lambda);
...
tx = txsite("cartesian", ...
    "Antenna",txArray, ...
    "AntennaPosition",[0; 25; 300],...%[-1.46; -1.42; 2.1], ...
    'TransmitterFrequency',5.8e9);

rx = rxsite("cartesian", ...
    "Antenna",rxArray, ...
    "AntennaPosition",[0;0;0],...%[.3; .3; .85], ...
    "AntennaAngle",[0;90]);
...
%   5. LDPC Decoding
[decBits, eqSym, chanEst] = ...
    helperIndoorRayTracingRxProcessingSIMO(rxWave,CIR, ...
...
%% Plot CSI? Still not sure if it's CSI
...
Hest_flat = reshape(Hest, [216, size(Hest, 2)*size(Hest,3)]);
x = (-size(Hest_flat, 1)/2:1:(size(Hest_flat,1)/2 - 1))';
figure; rows = 2; cols = 1; sgtitle("CSI Mag & Phase");
subplot(rows, cols, 1); title("CSI Magnitude"); plot(x, abs(Hest_flat)); xlabel("Subcarrier Index?"); ylabel("Magnitude (dB?)"); grid on;
subplot(rows, cols, 2); title("CSI Phase"); plot(x, rad2deg(angle(Hest_flat))); xlabel("Subcarrier Index?"); ylabel("Phase (deg)"); grid on; yticks(-180:30:180);