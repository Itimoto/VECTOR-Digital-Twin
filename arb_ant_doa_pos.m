%% Arbitrary Antenna DoA Positioning Example
% Builds on `base_doa_pos` by establishing an RX array with arbitrary
%   geometry, made up copies of 'measured' elements
% Here we have 4 elements. Every element is a stock monopole, with phase considered (?)

% Develops a Naive DoA estimate via the ULA Steering Equations
...
%% Generate the Antennas
% Isotropic TX Array
txArray = arrayConfig("Size",[1 numSTAant]);%,"ElementSpacing",spacing*lambda);

% Arbitrary RX Array <-- DOES NOT WORK YET
%{
aut = antenna.generateArbitraryAntenna(fc); % Use a stock antenna for now
%aut = design(dipole, fc);
%rx_elem = repmat(aut, 1, numAPant);
%rx_elem = {aut, aut2, aut3, aut4};
%rx_pos = [zeros(numAPant, 1), (-(numAPant-1)/2 : (numAPant-1)/2) * spacing*lambda, zeros(numAPant, 1)];
rx_pos = [zeros(1, numAPant); (-(numAPant-1)/2 : (numAPant-1)/2) * spacing*lambda; zeros(1, numAPant)];
% ^^ ULA, lambda-based spacing, centered at origin and stretched in the Y-axis
rxArray = phased.ConformalArray("Element", aut,...
                                "ElementPosition", rx_pos);
viewArray(rxArray);
%}
rx_elem = {design(monopole, fc), design(helix, fc), design(fractalCarpet, fc), design(cloverleaf, fc)};
rx_elem{2}.GroundPlaneRadius = 0.013;
rx_pos = [zeros(1, numAPant); (-(numAPant-1)/2 : (numAPant-1)/2) * spacing*lambda; zeros(1, numAPant)];
%rxArray = conformalArray('ElementPosition', rx_pos', 'Element', rx_elem);
...
rx.AntennaPosition = [0; 0; 0]; % Place RX Antenna at the origin  
...
disp(['Naive DoA estimated at [' num2str(mean(doa(1:end-1))) '] deg for true DoA ' num2str(beta_a) ' deg']);
