...
numSnapshots = 10; % Number of Snapshots for which we will generate CSI
...
%Hest_size = size(Hest);
%Hest = reshape(Hest, [Hest_size(2) Hest_size(3)]); % Select only a single antenna element
Hest_tmp = permute(chanEstActiveSC, [2 3 1]); % [TX, RX, SC]
Hest = zeros(size(chanEstActiveSC, 3), size(chanEstActiveSC, 1)); % Select the Subcarriers & AP Antennas only. Assume only one TX Antenna on STA
Hest(:, :) = Hest_tmp(1, :, :);
rxElemPos = helper.getElementPosition(rxArray); % Extract Element Positions
